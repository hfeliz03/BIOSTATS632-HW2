...
We are going to produce an R package with functions with documentation. The application is functionality to visualize and process data and estimates on family planning worldwide. Note that we already developed the function in HW1. We will further work on it and wrap it up in an R package in this HW2.
...
est <- read_csv("MyPackage/data/est-1.csv")
dat <- read_csv("MyPackage/data/contraceptive_use-1.csv")


x <- plot_cp(dat, est, iso_code = 4)
y <- plot_cp(dat, est, iso_code = 4, CI = NA)
z <- plot_cp(dat, est, iso_code = 404, CI = 80)
...
```{r visible = T, warning = F, include = T}
library(Rd2md)
# Parse the .Rd file into an object
rd_object <- tools::parse_Rd("MyPackage/man/plot_cp.Rd")

# Write the content of the Rd file into an md, then read it and render it
writeLines(as_markdown(rd_object), "plot_cp.md")
...
https://github.com/hfeliz03/BIOSTATS632-HW2
